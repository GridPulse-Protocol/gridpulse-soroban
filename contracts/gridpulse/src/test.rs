...
impl MockToken {
    ...
    pub fn transfer_from(env: Env, spender: Address, from: Address, to: Address, amount: i128) {
        ...
        let allow: Allow = env.storage().persistent().get(&akey).unwrap_or(Allow {
            amount: 0,
            until: 0,
        });
...
// The generated clients borrow from the `Env`, so they can't be stored in the
// same struct as the `Env` they were created from (that would be
// self-referential). Store the owned bits and build fresh clients on demand.
struct Fixture {
    env: Env,
    admin: Address,
    token_id: Address,
...
impl Fixture {
    ...
    fn gp(&self) -> GridPulseClient<'_> {
        GridPulseClient::new(&self.env, &self.gp_id)
    }
}
...
fn fixture(price: u64, fee_bps: u32) -> Fixture {
    ...
    MockTokenClient::new(&env, &token_id).init(&admin);
...
fn register_meter(fx: &Fixture, owner: &Address, seed: u8) -> (u64, SigningKey) {
    ...
    let id = fx.gp().register_meter(owner, &pk);
    (id, sk)
}

/// Submit a correctly-signed reading.
fn submit(fx: &Fixture, sk: &SigningKey, meter_id: u64, ts: u64, gen: u64, cons: u64, nonce: u64) {
